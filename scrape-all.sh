...
gnome-terminal --tab --title=PRPWorker -- bash -c "nice -+19 ./scrape-prps-worker.sh 2>&1 | tee /tmp/scrape-prps-worker.txt"
gnome-terminal --tab --title=PRP -- bash -c "digits=300 start=0 perpage=16 nice -+19 ./scrape-prps.sh 2>&1 | tee /tmp/scrape-prps.txt"
