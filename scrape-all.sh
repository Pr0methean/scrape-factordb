#!/bin/sh
echo 9 > /tmp/scrape-composites-large-threads
echo 3 > /tmp/scrape-composites-medium-threads
...
gnome-terminal --tab --title=PRPs -- bash -c "digits=300 start=0 perpage=3 nice -+19 ./scrape-prps.sh 2>&1 | tee /tmp/scrape-prps.txt"
