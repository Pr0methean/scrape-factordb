...
#	fi
        ...
        if [ ${now_hour_of_day} -lt 15 -a ${now_hour_of_day} -ge 1 ]; then
          # range of 96..106 when starting between 01:00 and 02:00 UTC
          # decreasing to 89..99 when starting between 14:00 and 15:00 UTC
          let "digits = 106 - (${now_hour_of_day} / 2) - ($job % 11)"
	...
	for num in $(shuf <<< ${exact_size_results}); do
          ...
          if flock -xn 9; then
              ...
              done < <(msieve -e -q -s "/tmp/msieve_${num}.dat" -fb "/tmp/msieve_${num}.fb" -t "${threads}" "${num}" | grep -o ':[0-9 ]\+' | grep -o '[0-9]\+' | head -n -1 | uniq)
              end_time=$(date +%s%N)
