...
while true; do
	...
	results=$(sem --id 'factordb-curl' -j 3 --fg xargs wget -e robots=off --no-check-certificate -t 10 -T 10 -nv -O- --retry-connrefused --retry-on-http-error=502 <<<"$url")
	for id in $(grep -o 'index.php?id=[0-9]\+' <<<"$results" |
                grep -o '[0-9]\+' |
		uniq); do
		let "restart = 0"
		echo "Checking ID ${id}"
		status=$(sem --id 'factordb-curl' -j 3 --fg xargs wget -e robots=off --no-check-certificate -t 10 -T 10 -nv -O- --retry-connrefused --retry-on-http-error=502 <<<"https://factordb.com/index.php\?id=${id}\&open=prime\&ct=Proof")
