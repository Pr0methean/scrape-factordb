...
results=$(sem --id 'factordb-curl' -j 4 xargs -n ${pertask} wget -e robots=off --no-check-certificate -nv -O- -T 30 -t 3 --retry-connrefused --retry-on-http-error=502 <<< "${assign_urls}" | grep '\(ssign\|queue\|>C<\|>P<\|>PRP<\)')
    echo $results
